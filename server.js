...
// [중요] 정적 파일 제공 설정을 맨 위로 올립니다.
// 이 설정 덕분에 프론트엔드에서 API 호출 시 주소를 생략하고 '/api/chat'만 써도 됩니다.
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        if (!currentConvId) {
            ...
            const [r] = await db.promise().query('INSERT INTO conversations (user_id, title) VALUES (?, ?)', [userId, title]);
        ...
        res.json({ reply, conversationId: currentConvId, cost: Math.round(cost * EXCHANGE_RATE * 100) / 100 });
    } catch (e) {
